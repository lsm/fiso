...
- **Fiso-Flow** (`cmd/fiso-flow`) — inbound pipeline: HTTP / Kafka / gRPC source → transform (CEL-compiled `fields`) → CloudEvent envelope → interceptor chain → HTTP / gRPC / Temporal / Kafka sink, with a DLQ.
- **Fiso-Link** (`cmd/fiso-link`) — outbound proxy the app calls at `localhost:3500/link/<target>`: circuit-breaker check → auth credentials → outbound interceptors → retry loop → HTTP or Kafka target, plus per-target rate limiting.
- **Fiso-Operator** (`cmd/fiso-operator`) — reconciles the `FlowDefinition` / `LinkTarget` CRDs (`api/v1alpha1`, manifests in `deploy/crds`) and injects sidecars via a mutating webhook.
...
E2E_BUILD_FLAG=" " bash test.sh          # reuse prebuilt fiso-flow:e2e / fiso-link:e2e images instead (what CI does)
...
**Flow builders are duplicated on purpose.** `buildPipeline` in `cmd/fiso-flow/main.go`, `cmd/fiso-flow-wasmer/main.go`, and `cmd/fiso-wasmer-aio/main.go` each contain the full source/sink/interceptor type switch. ADR 0003 says an integration value (source type, sink type, Link protocol, interceptor type) is supported only when validation, every builder, the operator reconciler (`internal/operator/reconciler.go`), the CRD enum, the docs, and a CI test all agree. Adding or removing a value in one place is a defect: update `internal/config.(*FlowDefinition).Validate`, all three builders, the reconciler, `deploy/crds/*.yaml`, README, and the `main_test.go` conformance tests together.

**Interceptor contract.** `internal/interceptor.Interceptor` processes a `Request` (payload + headers + direction) and may return `*RejectedError` (ADR 0007). A rejection is terminal: no retry, no DLQ, and request-response sources answer with its HTTP status and reason. Implementations live in `internal/interceptor/wasm` and `internal/interceptor/grpc`; Link has its own registry in `internal/link/interceptor`. Guest env vars arrive through `interceptors[].config.env` (ADR 0008) and must be delivered on both runtimes and in every Flow binary.
...
- Write an ADR (`docs/adr/`, template in its README, next zero-padded number) before changing a public contract, configuration model, cross-component architecture, or security/delivery/lifecycle semantics. Accepted ADRs are superseded by a new ADR, not rewritten.
...
- A PR states one observable claim, the verification performed and its result, what was not tested, and the rollback path. Keep a slice to one claim; do not bundle follow-ups.

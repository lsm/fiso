...
// stderrLimit bounds how much guest stderr is retained per invocation.
// The cap is enforced while capturing — not after — so a guest streaming
// unbounded diagnostics cannot grow host memory.
const stderrLimit = 512

// limitWriter retains at most limit bytes and reports truncation; writes
// beyond the cap are accepted (so the guest is never blocked) but dropped.
...
func (w *limitWriter) Write(p []byte) (int, error) {
	...
	_, _ = w.buf.Write(p)
	// Report the full length (io.Writer forbids a short nil write): the
	// dropped bytes are deliberately discarded, not failed.
	return original, nil
}

// withGuestStderr decorates an execution error with the guest's stderr
// diagnostic, when it wrote one.
func withGuestStderr(err error, stderr *limitWriter) error {
